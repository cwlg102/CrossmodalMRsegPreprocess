...
def main_func(fol_path,ind):
    ...
    for idx in range(item_num):
        ...
        if str(structureset[idx].ROIName)[0] == 'Z' or 'TV' in str(structureset[idx].ROIName) or 'nbl' in str(structureset[idx].ROIName):
            if 'Mattress' in str(structureset[idx].ROIName):
                the_name = str(structureset[idx].ROIName)
                pass
            else:
                continue
        ...
        for jdx in range(max_slices):
            ...
            if structureset[idx].ROIName == the_name:
                z_ctr_data["Mattress"].append(ctr_int_data)
    ...
    assert z_ctr_data["Mattress"] != [], "이름 check"

    if int(inf_ct.SliceThickness) == 3:
        slice_idx = 10
    elif int(inf_ct.SliceThickness) == 2:
        slice_idx = 15
    elif int(inf_ct.SliceThickness) == 1:
        slice_idx = 30

    
    y_val = sorted(z_ctr_data["Mattress"][-1], key=lambda x:x[1])[0][1]
    
    
    #이동 좌표 구하기
    cur_im_y_center = y_val
    print(y_val)
    dest_center = 360
    d_x, d_y = 0, dest_center-cur_im_y_center
    pad_param = 100

    #contour 이동 부분
    ctr_pad_vol = np.pad(ctr_vol, ((0, 0), (pad_param, pad_param), (pad_param, pad_param), (0, 0)), "constant")
    print(ctr_pad_vol.shape)
    ...
    # for i in range(len(ctr_moved_vol)):
        # img =Image.fromarray(ctr_moved_vol[i])
        # img.save(r"E:\debug\%d.jpg" %i)
    ...
    for idx in range(len(CT_list)):
        ...
        ct_arr = ct_pad_arr[pad_param-d_y:pad_param-d_y+512, pad_param-d_x:pad_param-d_x+512]
    ...
    for i in range(len(rev_t_vol)):
        ...
        im.save(os.path.join(r"E:\jpgtest","%d_%d.jpg" %(ind, i+1)))
        gray_im.save(os.path.join(r"E:\test_jpg_gray", "%d_%d.jpg" %(ind, i+1)))

basepath = r"E:\20211026 IDOL contouring study"
file_list = os.listdir(basepath)

for i in range(len(file_list)):
    try:main_func(r"E:\20211026 IDOL contouring study\%s\ct" %(file_list[i]), i+1)
    except:pass
    try:main_func(r"E:\20211026 IDOL contouring study\%s\ct1" %(file_list[i]), i+1)
    except:pass
    try:main_func(r"E:\20211026 IDOL contouring study\%s\ct2" %(file_list[i]), i+1)
    except:pass
    try:main_func(r"E:\20211026 IDOL contouring study\%s\ct3" %(file_list[i]), i+1)
    except:pass