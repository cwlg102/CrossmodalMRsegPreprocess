...
def main_func(fol_path,ind):
    ...
    for idx in range(item_num):
        ...
        for jdx in range(max_slices):
            ...
            if structureset[idx].ROIName == "Femur_Head_L" or structureset[idx].ROIName == "Lt Femur Head" or structureset[idx].ROIName == "Lt femur head":
                z_ctr_data["Femur_Head_L"].append(ctr_int_data)
            elif structureset[idx].ROIName == "Femur_Head_R" or structureset[idx].ROIName == "Rt femur head" or structureset[idx].ROIName == "Rt femur":
    ...
    for i in range(len(rev_t_vol)):
        ...
        im.save(os.path.join(r"D:\!Crossmodality_2023\안됐던거\jpgtest","%d_%d.jpg" %(ind, i+1)))
        gray_im.save(os.path.join(r"D:\!Crossmodality_2023\test_jpg_gray", "%d_%d.jpg" %(ind, i+1)))

basepath = r"D:\!Crossmodality_2023\안됐던거"
file_list = os.listdir(basepath)

hmm = [12, 34]
for i in range(len(file_list)):
    print(file_list[i])
    main_func(r"D:\!Crossmodality_2023\안됐던거\%s\ct" %(file_list[i]), hmm[i])
